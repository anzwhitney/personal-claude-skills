...
cumulative from infusion start. The phase boundaries (26:00 / 39:00) come from where the Max
Cooper run falls in the user's hand-curated reference sessions (see `curation-notes.md`). The
original 35:00 / 45:00 estimates put the Peak ~9 min late. Mirrored in code as `ytm.PHASES`.
...
|---|-------|--------|------|-----------|
| 1 | **Build** | 0:00-26:00 | ~26 min | Gradually intensifying ambient / downtempo. Calm, textural foundations early; density and energy rising steadily toward the peak. The long tail of the session. |
| 2 | **Peak** | 26:00-39:00 | ~13 min | The most *immersive* point, which isn't necessarily the most intense: immersive melodic electronic (Max Cooper or similar). Rhythmic/building is welcome here (not beatless). |
| 3 | **Wind-down** | 39:00-50:00+ | ~11 min+ (measured only to 50:00; may overflow past it) | About 3-4 gentle tracks for soft emergence, spanning the end of the active infusion into the comedown. The last track may carry vocals only if it starts at/after 47:00. |
...
- **Build** leans intensifying ambient/downtempo — this is the long tail of the session, so
  vary texture across it rather than looping one mood for 26 minutes.

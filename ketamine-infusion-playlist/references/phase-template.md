...
- **Instrumental only:** every track must have no lyrics at all, except possibly a single final
  track that *starts* at/after 47:00 (a deliberate vocal closer). Enforced mechanically by
  `build_playlist.py` (combined lyrics lookup + title heuristic) — see
...
- **Timing:** the playlist may run well past 50:00 — content after that point doesn't matter,
  ...
  only up to the 50:00 boundary; overflow past it is never penalized.
- **Artist diversity (whole playlist):** prefer consecutive tracks by different artists, and
  ...
  both the consecutive-artist and per-artist-cap rules are relaxed there.
...
- Timing precision matters to the user; keep phase windows tight (see
  `PHASE_TOLERANCE_SECONDS` in `scripts/ytm.py` for the automated tolerance check).
